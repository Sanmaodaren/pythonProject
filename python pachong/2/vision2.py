...
def write_at_once():
    ...
    for i in range(1, 21):
        ...
        for b in range(2,6):
            for a in list2:
                ...
                b += 4
    workbook = xlrd.open_workbook('ljz.xls', formatting_info=True)
    ...
    for xq, in_dic in weeks.items():  # 总字典，所有的信息都在里面，{星期：{节次：周数集合}}
        ...
        for week_b, week_cell in in_dic.items():  # 提取出{节次+1：周数集合}中的key和value
            ...
            for x in week_cell_list:
                sheet = wb.get_sheet(f'sheet{x}')
                sheet.write(int(week_b)+1, int(xq), '闫顺')
    workbook = xlrd.open_workbook('yangguang.xls', formatting_info=True)
    worksheet = workbook.sheet_by_name('sheet1')
    get_info_from_old_excel(worksheet)
    weeks = dic_weeks_all_cols
    for xq, in_dic in weeks.items():  # 总字典，所有的信息都在里面，{星期：{节次：周数集合}}
        ...
        for week_b, week_cell in in_dic.items():  # 提取出{节次+1：周数集合}中的key和value
            ...
            for x in week_cell_list:
                sheet = wb.get_sheet(f'sheet{x}')
                sheet.write(int(week_b)+2, int(xq), '杨光')
    workbook = xlrd.open_workbook('yangjian.xls', formatting_info=True)
    worksheet = workbook.sheet_by_name('sheet1')
    get_info_from_old_excel(worksheet)
    weeks = dic_weeks_all_cols
    for xq, in_dic in weeks.items():  # 总字典，所有的信息都在里面，{星期：{节次：周数集合}}
