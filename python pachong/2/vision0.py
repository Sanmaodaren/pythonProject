    ...
    for xq, in_dic in weeks.items():  # 总字典，所有的信息都在里面，{星期：{节次：周数集合}}
        ...
        for week_b, week_cell in in_dic.items():  # 提取出{节次+1：周数集合}中的key和value
            ...
            for x in week_cell:
                ...
                sheet.write(int(week_b), int(xq), f'{name}')
    wb.save(f'无课表{filename}')


def get_info_from_old_excel():
    ...
    write_at_once(dic_weeks_all_cols)


if __name__ == '__main__':
    get_info_from_old_excel()